...
on a spec thread. Counted by script rather than by hand, as of 2026-08-30: **52 flap
events across 41 hosts in 23 days, 104 recorded transitions that are not state changes**, and 23
of the 52 land on a single day, all under one parent domain. The committed `flap_census.json` is
regenerated by the daily job, so the artifact and the series never drift apart; run
`python flap_census.py` to recount it from the snapshots yourself. The sweep now retries a
...
hosts the index lists that the frozen cohort cannot see. `python verify_rates.py` does the same for
the host-level and operator-level change rates (0.53% over 6,084 host-days, 0.87% over 2,652
operator-days), including the denominator collapse that makes the operator rate the higher of the two.

**The measurement is itself checkable.** Twenty-three consecutive days as of 2026-08-30, no gaps. Each
